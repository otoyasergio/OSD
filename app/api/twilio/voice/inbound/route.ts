...
import { handleInboundVoice } from "@/lib/services/shopPhone";
...
export async function POST(request: Request) {
  ...
  try {
    const xml = await handleInboundVoice(parsed.params);
    return twimlResponse(xml);
  } catch (error) {
    captureException(error, { requestId, route: "twilio-voice-inbound" });
    logger.error("Twilio inbound voice failed", {
      requestId,
      error: error instanceof Error ? error.message : "VOICE_INBOUND_FAILED",
    });
