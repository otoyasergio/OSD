-- Board/gallery cards use a small JPEG; lightbox and inspection still
-- open the original storage_path so staff can see scratches, VIN, and fasteners.

ALTER TABLE public.intake_photo
  ADD COLUMN IF NOT EXISTS thumb_storage_path text;

COMMENT ON COLUMN public.intake_photo.thumb_storage_path IS
  'Optional ~480px JPEG for boards and grids. Null on photos uploaded before thumbs existed.';

DROP FUNCTION IF EXISTS public.board_primary_intake_photos(uuid[]);

CREATE FUNCTION public.board_primary_intake_photos(p_work_order_ids uuid[])
RETURNS TABLE (
  work_order_id uuid,
  storage_path text,
