...
import { canSyncWixContacts } from "@/lib/permissions";
...
async function linkLocalCustomerToWix(
  supabase: AdminClient,
  customer: CustomerMatchRow
): Promise<string> {
  const profile = sanitizeWixPushProfile(customer);
  ...
  const { data: other } = await supabase
    ...
    .maybeSingle();
  if (other) throw new Error("WIX_CONTACT_ALREADY_LINKED");
 ...
 */
export async function syncCustomerToWix(customerId: string): Promise<CustomerWithWix> {
  const user = await requireUser();
  if (!canSyncWixContacts(user.role)) throw new Error("FORBIDDEN");
  ...
  const profile = sanitizeWixPushProfile({
    customer_id: customer.customer_id,
    first_name: customer.first_name,
    last_name: customer.last_name,
    email: customer.email,
    phone: customer.phone,
    wix_contact_id: customer.wix_contact_id,
  });
...
export async function maybeSyncCustomerToWix(customerId: string): Promise<void> {
  ...
  try {
    ...
    logger.error("Wix customer push failed", {
      customerId,
      error: error instanceof Error ? error.message : "WIX_SYNC_FAILED",
    });
  }
}
...
async function reconcileWixContactsToAppBody(
  ...
  for (const contact of wixContacts) {
    ...
    try {
      ...
      if (result.created) {
        ...
        stats.updated += 1;
        const idx = localRows.findIndex((row) => row.customer_id === result.customer_id);
        if (idx >= 0) {
          localRows[idx] = {
            ...localRows[idx],
            first_name: fields.firstName,
  ...
  for (const local of customersNeedingWixPush(localRows)) {
    if (!local.email && !local.phone) {
      stats.skipped += 1;
      continue;
    }
    try {
      const wixContactId = await linkLocalCustomerToWix(supabase, local);
      stats.pushed += 1;
      const idx = localRows.findIndex((row) => row.customer_id === local.customer_id);
      if (idx >= 0) {
        localRows[idx] = { ...localRows[idx], wix_contact_id: wixContactId };
      }
    } catch (error) {
      if (error instanceof Error && error.message === "WIX_CONTACT_ALREADY_LINKED") {
        stats.skipped += 1;
      } else {
        stats.failed += 1;
      }
...
export async function pullCustomerFromWix(customerId: string): Promise<CustomerWithWix> {
  const user = await requireUser();
  if (!canSyncWixContacts(user.role)) throw new Error("FORBIDDEN");
  if (!isWixContactsConfigured()) throw new Error("WIX_NOT_CONFIGURED");
  ...
  const firstName = remote.info?.name?.first?.trim() || customer.first_name;
  const lastName = remote.info?.name?.last?.trim() || customer.last_name;
  const email =
